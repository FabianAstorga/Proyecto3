...
export class ActivitiesHistoryComponent implements OnInit {
  ...
  // Opciones del selector de meses
  meses = [
    { value: '1', label: 'Enero' },
  ...
  /** Etiqueta para el título (usa el mes actual si existe) */
  get currentMonthLabel(): string {
    const m = this.currentMonthGroup;
    return m ? m.monthLabel : 'Historial';
  }
  ...
  downloadMonthPdf(month: MonthGroup | null): void {
    if (!month) return;
